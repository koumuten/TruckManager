...
void main() {
  ...
  group('Integration Test with Mock Data', () {
    test('should generate correct JSON for LINE notification', () async {
      ...
      final messageListJson = jsonDecode(await messageListFile.readAsString())
      ...
      final messageDetailFile =
          File('test/static_files/email_get_$messageId.json');
      final messageDetailJson =
          jsonDecode(await messageDetailFile.readAsString())
              as Map<String, dynamic>;
      ...
      final messageDetail = gmail.Message()
        ...
        ..payload = (gmail.MessagePart()
          ..parts = ((messageDetailJson['payload']
            ...
            return gmail.MessagePart()
              ...
              ..body = (gmail.MessagePartBody()
                ..attachmentId =
                    (partJson['body'] as Map<String, dynamic>)['attachmentId']);
          }).toList());

      // 2番目のメールのモック
      final messageId2 = messages[1].id!;
      final messageDetailFile2 =
          File('test/static_files/email_get_$messageId2.json');
      final messageDetailJson2 =
          jsonDecode(await messageDetailFile2.readAsString())
              as Map<String, dynamic>;
      final messageDetail2 = gmail.Message()
        ..id = messageDetailJson2['id']
        ..payload = (gmail.MessagePart()
          ..parts = ((messageDetailJson2['payload']
                  as Map<String, dynamic>)['parts'] as List)
              .map((part) {
            final partJson = part as Map<String, dynamic>;
            return gmail.MessagePart()
      ...
      when(mockGmailService.fetchMessageList(any))
          .thenAnswer((_) async => messages);
      when(mockGmailService.getMessageDetails(messageId))
          .thenAnswer((_) async => messageDetail);
      when(mockGmailService.fetchAttachment(messageId, any))
          .thenAnswer((_) async => [1, 2, 3]); // ダミーのPDFバイト
      when(mockGmailService.markAsRead(messageId))
          .thenAnswer((_) async => Future.value());

      when(mockGmailService.getMessageDetails(messageId2))
          .thenAnswer((_) async => messageDetail2);
      when(mockGmailService.fetchAttachment(messageId2, any))
          .thenAnswer((_) async => [1, 2, 3]); // ダミーのPDFバイト
      when(mockGmailService.markAsRead(messageId2))
          .thenAnswer((_) async => Future.value());

      // PDFから抽出されるテキストをモック
      final pdfText = '発行日: 2024/05/20\nお支払い総合計: 50,000円';
      when(mockPdfService.extractTextFromPdf(any))
          .thenAnswer((_) async => pdfText);

      // スプレッドシートからのデータをモック
      final shiftCapsule = ShiftCapsule(
          date: DateTime(2024, 5, 20),
          assignment: 'Truck-X',
          eventName: 'テスト運搬');
      when(mockGoogleSheetsService.retriveTruckData(any))
          .thenAnswer((_) async => shiftCapsule);

      // PDFからJPGへの変換処理をモック
      final invoiceCapsule = InvoiceCapsule(
          date: '2024-05-20',
          price: '¥50,000',
          invoiceImgPath: '/tmp/invoice.jpg');
      ...
      when(mockFirebaseService.uploadFile(
              storagePath: anyNamed('storagePath'), file: anyNamed('file')))
          .thenAnswer((_) async => downloadUrl);
      when(mockFirebaseService.saveDocument(
              collectionId: anyNamed('collectionPath'),
              docId: anyNamed('docId'),
              data: anyNamed('data')))
          .thenAnswer((_) async => 'success_doc_id');

      // 3. ワークフローの実行
      final results = await appService.runInvoiceSyncWorkflow(
          targetSubject: 'test', isDebug: true);

      // 4. 結果のJSONを解析
      final decodedBody = jsonDecode(results);
      final carouselMessages =
          decodedBody['messages'][0]['contents']['contents'];
